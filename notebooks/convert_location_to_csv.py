"""
convert_location_to_csv.py
===========================
Chuyển file location.txt thành location.csv với định dạng:
    filename, full_plate, province, series, number

Format dòng trong location.txt:
    <stt> <filename> <cls> <x> <y> <w> <h> <plate_parts...>
...
# Tất cả mã tỉnh xe máy VN (2 chữ số, 11–99)
PROVINCE_CODES = {
...
def clean_token(token: str) -> str:
    """Loại bỏ ký tự noise, chỉ giữ alphanumeric."""
...
def parse_plate(tokens: list[str]) -> tuple[str, str, str, str]:
    ...
    Quy tắc:
      - Province: 2 chữ số đầu tiên (có trong PROVINCE_CODES)
      - Series:   phần còn lại của token đầu sau province (thường 1-2 ký tự)
      - Number:   toàn bộ token còn lại, nối liền

    Returns:
        (full_plate, province, series, number)
    """
    if not tokens:
        return '', '', '', ''
    ...
    # ── 2) Series = token đầu còn lại ──
    if cleaned:
        series = cleaned[0].upper()

    # ── 3) Number = token còn lại, nối liền ──
    if len(cleaned) > 1:
        number = ''.join(cleaned[1:])
    elif len(cleaned) == 1 and cleaned[0].isdigit():
        # Trường hợp series bị missing trong annotation
        number = cleaned[0]
        series = ''
